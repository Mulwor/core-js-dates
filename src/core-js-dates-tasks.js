...
function getNextFriday(date) {
  ...
  if (getDay < 5) {
    return new Date(date.setDate(getDate + (5 - getDay)));
  }

  return new Date(date.setDate(getDate + (7 - getDay + 5)));
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const countingDay = 1000 * 60 * 60 * 24;
  const start = new Date(dateStart).getTime();
  const end = new Date(dateEnd).getTime();

  return Math.floor((end - start) / countingDay + 1);
}
